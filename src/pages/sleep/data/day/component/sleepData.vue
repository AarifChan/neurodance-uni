...
watch(
  ...
  (newVal) => {
    if (newVal) {
      let hour = Math.floor(newVal.durationMin / 60)
      let min = Math.floor(newVal.durationMin % 60)
      const startTime = formatTimestamp(newVal.sleepStart, 'HH:mm')
      const endTime = formatTimestamp(newVal.sleepEnd, 'HH:mm')
      const startDate = formatTimestamp(newVal.sleepStart, 'MM/DD')
      const endDate = formatTimestamp(newVal.sleepEnd, 'MM/DD')
      item.value = {
        hour: hour,
        minute: min,
        state: '正常',
        isExtend: 1, // 0: 无变化、1: 增加、2: 减少
        id: newVal.sleepId, // 当前id
        startTime: startTime, // 开始时间 - 时分
        endTime: endTime, // 结束时间 - 时分
